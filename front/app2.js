...
createApp({
    ...
    methods: {
        ...
        async fetchBookDetail(book) {
            if (!book.id) {
                showToast("获取不到书籍编码", "error");
                return;
            }

            // 先打开弹窗，显示正在加载
            this.currentBookDetail = { title: book.title, author: book.author, info: {}, holdings: [] };
            this.loadingBook = true;
            this.showBookModal = true;

            try {
                const res = await fetch(`${API_BASE}/book_detail`, {
                    method: "POST",
                ...
                if (res.ok) {
                    // 把后端传回来的 info 和 holdings 合并进去
                    this.currentBookDetail = { ...this.currentBookDetail, ...result.data };
                } else {
        ...
        async searchLibrary() {
            if (!this.bookKeyword.trim()) {
                showToast("请输入想要搜索的书名", "error");
                return;
            }
            this.isSearchingBooks = true;
            this.bookList = []; // 搜索前清空上次的结果

            try {
                const res = await fetch(`${API_BASE}/search_books`, {
                    method: "POST",
                    headers: { "Content-Type": "application/json" },
                    body: JSON.stringify({ keyword: this.bookKeyword })
                });
                const result = await res.json();
                ...
                if (res.ok) {
                    ...
                    if (this.bookList.length === 0) {
                        showToast("没有找到相关的书籍", "error");
                    }
                } else {
                    showToast("搜索失败，请稍后再试", "error");
                }
            } catch (e) {
                showToast("网络连接异常，无法连接图书馆", "error");
            } finally {
